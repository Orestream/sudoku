 ...
 */
function buildColoredComponents(grid: SolverGrid, digit: number): Map<number, Color>[] {
 ...
 */
function findUncoloredCells(grid: SolverGrid, digit: number, coloredCells: Set<number>): number[] {
	const bit = 1 << (digit - 1);
